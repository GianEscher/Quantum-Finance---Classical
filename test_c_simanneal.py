...
def simanneal(gain, volatility, risks):
    E = 2              #number os steps or λ values
    ...
    while(CRITERIO_PARADA > 0.0005):
      ...
      CRITERIO_PARADA = np.std(data['Risco'])
...
def plot_markowitz(data, testdict, x):
  ...
  plt.savefig('testeagora/portfolios'+ str(x) +'.png')
  ...
  plt.savefig('testeagora/selection'+ str(x) +'.png')
...
def interface():
    ...
    while(risk_delta > 1):
        ...
        risk_delta = 0#np.std(data['Risco'])
        #print('risk_delta: ' + str(risk_delta))
        #print('lowest_for_test: ' + str(lowest_for_test))
    ...
    print(overview)
    print('\n \n número de iterações: ' + str(ITERATIONS_TOTAL))
    
    overview.to_csv('c_results.csv')
