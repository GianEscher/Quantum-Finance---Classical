...
def setup(n_stocks, data_inicial, data_final, codes):
  ...
  plt.rcParams["figure.figsize"] = [20.50, 18.50]
  ...
  while(i<len(volatility)):
    ...
    while(j<len(volatility)):
      if(i==j):
        ...
        row.append(variat)
        ax.text(i ,j , f"{variat*1000:.3f}", fontsize=10, va = 'center', ha = 'center')
        
      else:
        covariat = covar(stocks_registry[i],gain[i],stocks_registry[j],gain[j])
        row.append(covariat)
        ax.text(i ,j , f"{covariat*1000:.3f}", fontsize = 10, va = 'center', ha = 'center')
  ...
  ax.set_title(" Covariâncias populacionais e Variâncias (.10^-3)")
  ax.set_xlabel(str(codes), fontsize = 11)
  plt.imshow(risks, interpolation='nearest', cmap="coolwarm")
  plt.colorbar()
  #plt.show()
...
def markowitz(n_stocks, codes, gain, volatility, risks):
  ...
  while(iter<iterations):
    ...
    sum_weigths = 0
    while(i<n_stocks):
      new_weight = np.random.uniform(low=0.01, high=1.0, size=(1,))[0]
      sum_weigths += new_weight
    ...
    sum_a = 0
    while(i<n_stocks):
      current_weight = weights[i]/sum_weigths
      sum_a += current_weight*current_weight*volatility[i]*volatility[i]
    ...
    risk_total.append(current_risk)
